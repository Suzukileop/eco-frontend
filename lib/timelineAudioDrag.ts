...
export interface AudioDragPreview {
  clipId: string;
  targetLane: number;
  startTime: number;
...
export interface AudioDragGhost {
  clipId: string;
  clientX: number;
  clientY: number;
  grabDx: number;
...
export {
  ...
  clampBackgroundDragPointer as clampAudioDragPointer,
...
export type ClampedAudioDragPointer = ClampedBackgroundDragPointer;

/** Résolution de lane au drag — empilement vers le bas (inverse texte/overlay). */
export function resolveAudioLaneForDrag(
  clientY: number,
  sourceLane: number,
  ...
  const rows = Array.from(
    document.querySelectorAll<HTMLElement>('[data-audio-lane-index]')
  );
  ...
  const rowInfo = rows.map((row) => {
    ...
    return {
      lane: Number(row.dataset.audioLaneIndex ?? PRIMARY_AUDIO_LANE),
      top: rect.top,
      bottom: rect.bottom,
    };
  });
...
export function buildAudioDragPreview(
  clip: Clip,
  allAudio: Clip[],
  targetLane: number,
  ...
  const laneClips = allAudio.filter((c) => getClipAudioLane(c) === targetLane);
